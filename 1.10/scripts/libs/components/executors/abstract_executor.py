...
class AbstractExecutor(ABC):
    ...
    def __init__(self, tool_manager) -> None:
        ...
        if SystemHandler().os_system.platform_name != "windows":
            ...
            self.signalMap[signal.SIGTERM] = self.handle_memory_emergency
        self.threads = []
    ...
    def watch_threads(self, end_time):
        ...
        while True:
            ...
            if (
                ...
                LogManager().log(
                    "SYS", LogManagerThread.Level.INFO, timeout_msg
                )

                LogManager().handle_emergency_memory_analysis()

                LogManager().set_preserve_loggers(["IMC", "SYS"])
        ...
        while not queue_emptied:
            try:
                ...
                if SystemHandler().os_system.platform_name.upper() == "WINDOWS":
                    for taskkill_result in self.tool_manager.tool_data.data[
                ...
                self.tool_manager.tool_data.data["data_from_queue"].append(
                    current_data
                )
            except queue.Empty:
                queue_emptied = True
    ...
    def halt_threads(self, *args):
        ...
        for curr_thread in self.live_threads():
            if curr_thread.proc:
                SystemHandler().os_system.halt_thread(curr_thread)
    ...
    def resume_threads(self, *args):
        ...
        for curr_thread in self.live_threads():
            if curr_thread.proc:
                SystemHandler().os_system.resume_thread(curr_thread)
    ...
    def handle_memory_emergency(self, signum, frame):
        """
        This signal handler method provides emergency response for critical system
        conditions. It serves as a safety mechanism to ensure diagnostic
        data is captured when unexpected termination signals are received during
        memory-intensive operations.

        Behaviour:
            - Immediately stops all active execution threads
            - Triggers emergency memory analysis through LogManager
            - Performs graceful system exit to preserve diagnostic state

        Args:
            signum (int): The signal number that triggered the emergency handler.
            frame (frame): The current execution stack frame at the time the signal
                was received.

        Returns:
            None: This method does not return as it terminates the process after
                completing emergency cleanup operations.

        """
        if signum == signal.SIGTERM:
            self.stop_threads(signum, frame)
            time.sleep(0.1)
            sys.exit(0)
