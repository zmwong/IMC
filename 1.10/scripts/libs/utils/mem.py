...
def read_meminfo() -> dict:
    ...
    try:
        LogManager().log(
            "SYS", LogManagerThread.Level.DEBUG, f"Reading {file_info}"
        )
        ...
        LogManager().log("SYS", LogManagerThread.Level.WARNING, str(ex_err))
    return _meminfo
...
def get_system_info(attribute: str) -> int:
    ...
    try:
        ...
        LogManager().log("SYS", LogManagerThread.Level.CRITICAL, str(ex_err))
    return value
...
def get_svos_info(attribute: str) -> int:
    ...
    value = 0
    pattern = (
        r"^(?P<label>\w+)\s+"  # label: one or more word characters at the start, followed by whitespace
        r"(?P<desc>.+?)\s*=\s*"  # desc: minimal match of any character(s) up to '=', allowing optional spaces before/after '='
        r"(?P<value>.+?)"  # value: minimal match of any character(s) after '='
        r"(?:\s+\(.+\))?$"  # optional: whitespace, then parentheses with any content, at the end of the line
    )
    try:
        svos_info = (
        ...
        for line in svos_info:
            ...
            if not match:
                continue
            key = f"{match.group('label')}{match.group('desc')}"
            key = re.sub(r"[\W_]+", "", key)
            value = match.group("value").strip()
            _svos_info[key] = value
        value = (
