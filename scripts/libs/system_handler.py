...
class SystemHandler(metaclass=SingletonMeta):
    ...
    def __init__(self, argv=None) -> None:
        ...
        LoggerManager().create_logger(
            name=self.logger_name, log_level=log_level
        )
    ...
    def _get_log_level_from_verbosity(self, verbosity):
        """
        Maps a verbosity level to a LoggerManagerThread.Level.
    ...
    def init_os_system(self):
        ...
        if system not in ["windows", "linux", "svos"]:
            ...
            self.os_system = LazyLoader.load(
                f"scripts.libs.components.os_system.{system}",
                f"{system.title()}System",
            )()
