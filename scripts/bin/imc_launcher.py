...
import libs.utils.arg_support as argspt
from libs.utils import numa
from libs.definitions.imc import BinaryNames, MEM_USE_DFLT
from libs.test_generator import generator_main
from libs.utils.logging import Level
from libs.definitions.paths import DefaultPaths
from libs.utils.environment import EnvironmentInfo
from libs.utils.arg_support import ALLOCATOR, ALLOW_ALIASING, PHYSICAL_ADDRESS
from libs.utils.arg_support import PHYSICAL_ADDRESS_HIGH, PHYSICAL_ADDRESS_LOW
from libs.utils.arg_support import TARGET_PATH, BLOCK_SIZE, BLOCKS_AMOUNT
from libs.utils.arg_support import ENABLE_EXEC_MAPPING, ENABLE_WRITE_MAPPING
from libs.utils.arg_support import MEM_USAGE, MAPPING_MODE, ENABLE_READ_MAPPING
from libs.test_generator import generator_parser
from libs.definitions.imc import ParameterIdentifier
from libs.definitions.imc import IMCMemory, TOOL_NAME
from libs.definitions.exit_codes import ExitCode
from libs.utils.arg_support import VerboseAction
from scripts.libs.test_generator.menu import generate_tests_via_menu
from libs.utils.paths import fix_full_path_from_root
from libs.components.loggers.logger_manager import (
    LoggerManager,
    LoggerManagerThread,
)
...
def _call_runner(files, binary, runner_cmd, imc_runner_path):
    ...
    for file in files:
        ...
        LoggerManager().log(
            "IMC", LoggerManagerThread.Level.INFO, f"Testing: {file}"
        )
        row["Runner execution"] = file
        # Executing Runner
        cmd_script = [
...
def _compile(environment_info: EnvironmentInfo, parsed_args):
    ...
    if parsed_args.enable_compilation_logs:
        LoggerManager().log("IMC", LoggerManagerThread.Level.INFO, stdout)
        LoggerManager().log("IMC", LoggerManagerThread.Level.INFO, stderr)
    if status_code != 0:
        LoggerManager().log(
            "IMC", LoggerManagerThread.Level.ERROR, "Failed at compiling tool."
        )
    else:
        LoggerManager().log(
            "IMC", LoggerManagerThread.Level.INFO, "Compilation finished"
        )
