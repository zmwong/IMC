...
def parse_args_list(argv: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="queue",
        choices=["queue", "batch"],
        help="Specify the thread execution type, either queue or batch, both options use the number of lpus in the system as the number of threads to be executed. "
        + " Queue: Each thread will take a test case from the queue and execut an IMC subprocess until"
        + " the queue is empty. Batch: The number of lpus is used to split the test cases"
        + " into batches, where each thread recieves a test case from the batch, once the whole"
        + " batch has completed execution the next batch is processed. Default is %(default)s.",
    )
    parser.add_argument(
        "-v",
        dest="verbosity",
